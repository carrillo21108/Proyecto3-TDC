...
class Tape(object):
    ...
    def get_id(self,head_position,current_control):
        ...
        for i in range(min_index, max_index+1):
            if i==head_position:
                str_tape += '\033[1;33m' + '[' + current_control[0] + ',' + current_control[1] + ']' + '\033[0m'
...
class TuringMachine(object):
    ...
    def __init__(self, tape = '', alphabet = set(), tape_alphabet=set(), states=set(), blank_symbol = ' ', initial_control = tuple(), final_states = set(), transition_function = dict()):
    ...
    def step(self):
        ...
        if control[1] in self.tape_alphabet and control[0][0] in self.states:
    ...
    def final(self):      
        if self.current_control[0] in self.final_states:
            __id = self.tape.get_id(self.head_position,self.current_control)
...
def get_data():
   with open('mt-reconocedora.yaml', 'r') as f:
...
def get_TuringMachine(tape,data):
    ...
    for item in data['delta']:
        ...
        if item['params']['mem_cache_value']==None:
            item['params']['mem_cache_value'] = ' '
            
        if item['params']['tape_input']==None:
            item['params']['tape_input'] = ' '
            
        if item['output']['mem_cache_value']==None:
            item['output']['mem_cache_value'] = ' '
            
        if item['output']['tape_output']==None:
            item['output']['tape_output'] = ' '
            
        transition_function[((item['params']['initial_state'],item['params']['mem_cache_value']),item['params']['tape_input'])] = ((item['output']['final_state'],item['output']['mem_cache_value']), item['output']['tape_output'],item['output']['tape_displacement'])
    
    if data['tape_alphabet'][-1]==None:
        data['tape_alphabet'][-1] = ' '
    ...
    return TuringMachine(tape = tape,
                      ...
                      initial_control = (data['q_states']['initial'],data['tape_alphabet'][-1]),
